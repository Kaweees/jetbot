...
5. Install Jupyter Clickable Image widget (to support road following example).
    ...
    sudo pip3 install -e . && \
    sudo env "PATH=$PATH" jupyter labextension install js && \
    sudo env "PATH=$PATH" jupyter lab build
    cd ..
    ```
...
6. Install other miscellaneous dependencies
    ...
    ```bash
    sudo apt-get update && sudo apt-get install -y supervisor unzip
    sudo apt install -y python3-smbus && sudo pip3 install pyzmq
    ```
...
1. Create the service file at ``/etc/systemd/system/jetbot_jupyter.service``.

    ```bash
    sudo vim /etc/systemd/system/jetbot_jupyter.service
    ```
...
2. Enable the system service to run at boot
...
1. Create the service file at ``/etc/systemd/system/jetbot_display.service``.

    ```bash
    sudo vim /etc/systemd/system/jetbot_display.service
    ```
